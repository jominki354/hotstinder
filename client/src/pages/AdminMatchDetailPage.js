    ...
    <div className="max-w-6xl mx-auto px-4 py-8 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div>
                      {matchData.winner ? (
                        <span className={matchData.winner === 'blue' ? 'text-blue-400' : 'text-red-400'}>
                          {matchData.winner === 'blue' ? '블루 팀' : matchData.winner === 'red' ? '레드 팀' : matchData.winner}
                        </span>
                      ) : (
                ...
                {matchData.blueTeam?.map((player, index) => (
                  <li key={index} className="bg-blue-900/30 p-2 rounded flex justify-between items-center">
                    <div className="flex items-center">
                      <div>
                        <span className="text-white font-medium">{player.battletag || player.nickname}</span>
        ...
        <div className="lg:col-span-1">
          ...
          <div className="bg-slate-800 rounded-lg p-6 shadow-xl">
            <h2 className="text-xl font-semibold text-white mb-4">MMR 영향</h2>
            <div className="space-y-2">
              <div className="flex justify-between">
                <span className="text-gray-400">상태:</span>
                <span className={getStatusStyle(matchData.status)}>
                  {matchData.status === '무효' ? '무효 (MMR 영향 없음)' : matchData.status}
                </span>
              </div>
              {matchData.mmrChanges && matchData.mmrChanges.length > 0 ? (
                <>
                  <div className="text-gray-400 mt-3 mb-1">플레이어별 MMR 변화:</div>
                  <div className="bg-slate-700/50 p-3 rounded max-h-40 overflow-y-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="text-gray-400 border-b border-slate-600">
                          <th className="text-left p-1">플레이어</th>
                          <th className="text-right p-1">변화</th>
                        </tr>
                      </thead>
                      <tbody>
                        {matchData.mmrChanges.map((change, index) => (
                          <tr key={index} className="border-b border-slate-700/50">
                            <td className="p-1 text-white">{change.battletag || change.userId}</td>
                            <td className={`p-1 text-right ${change.change > 0 ? 'text-green-400' : change.change < 0 ? 'text-red-400' : 'text-gray-400'}`}>
                ...
                <p className="text-gray-400">MMR 변화 기록이 없습니다.</p>
              )}
            </div>
          </div>
        </div>
