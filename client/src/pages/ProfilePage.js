...
const ProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('프로필 업데이트 시도:', formData);
      ...
      if (response.data.success) {
        ...
        const timestamp = new Date().getTime();
        const updatedUser = await axios.get(`http://localhost:5000/api/auth/user?t=${timestamp}`, {
          withCredentials: true
        });
        ...
        {isEditing ? (
          ...
          <div>
            <div className="mb-4">
              ...
              <p className="text-white">{user?.battletag || '설정된 배틀태그가 없습니다.'}</p>
            </div>
