...
const AdminMatchesPage = () => {
  ...
  const showConfirmModal = (action, matchId) => {
    ...
    setConfirmAction({
      ...
      isMultiple: !matchId && (action === 'delete' || action === 'invalidate')
    });
  };
            ...
            <tbody>
              {matches.map((match) => (
                <tr key={match._id} className="border-t border-slate-700 hover:bg-slate-700/50">
                  ...
                  <td className="px-4 py-3">
                    {match.winner ? (
                      <span className={match.winner === 'blue' ? 'text-blue-400' : 'text-red-400'}>
                        {match.winner === 'blue' ? '블루 팀' : match.winner === 'red' ? '레드 팀' : match.winner}
                      </span>
                    ) : (
