...
module.exports = async function handler(req, res) {
  try {
    console.log('Vercel /api/admin/create-test-matches 요청 처리');
    ...
    const matchCount = Math.min(Math.max(1, parseInt(count)), 50); // 1-50개 제한

    console.log(`${matchCount}개의 테스트 매치 생성 시작`);

    // 사용 가능한 사용자들 가져오기 (최소 10명 필요)
    ...
    for (let i = 0; i < matchCount; i++) {
      try {
        ...
        // MMR 차이에 따른 승률 계산 (하지만 여전히 랜덤 요소 포함)
        const mmrDiff = team0AvgMMR - team1AvgMMR;
        const team0WinChance = 0.5 + (mmrDiff / 1000) * 0.2; // MMR 차이 1000당 20% 승률 변화
    ...
    res.status(200).json({
      ...
      matches: createdMatches.slice(0, 10) // 처음 10개만 반환
    });
