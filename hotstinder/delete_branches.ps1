# 메인 브랜치를 제외한 모든 원격 브랜치 삭제 PowerShell 스크립트
Write-Host "GitHub 원격 레포지토리에서 모든 브랜치 삭제를 시작합니다 (main 제외)" -ForegroundColor Green
Write-Host "----------------------------------------------------" -ForegroundColor Gray

# 사용자 확인
Write-Host "경고: 이 작업은 되돌릴 수 없으며, 'main' 브랜치를 제외한 모든 원격 브랜치가 삭제됩니다." -ForegroundColor Yellow
...
$branches = git branch -r | Where-Object { $_ -match "origin/" -and $_ -notmatch "origin/main" -and $_ -notmatch "origin/HEAD" }
