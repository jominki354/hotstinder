...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    const matches = await Match.find(query)
      .populate('creator', 'battleTag nickname profilePicture')
      .sort('-createdAt')
      .skip(skip)
...
router.get('/:id', authenticate, async (req, res) => {
  try {
    ...
    // 비공개 매치인 경우 접근 권한 확인
    if (match.isPrivate) {
      const isCreator = match.creator._id.toString() === req.user._id.toString();
      const isParticipant = 
    ...
    if (winner === 'blue' || winner === 'red') {
      ...
      for (const player of winnerTeam) {
        ...
        if (user) {
          ...
          user.playerStats.mmr += 15; // MMR 증가
          await user.save();
        }
      }
      ...
      for (const player of loserTeam) {
        ...
        if (user) {
          ...
          user.playerStats.mmr = Math.max(1000, user.playerStats.mmr - 10); // MMR 감소 (최소값 보장)
          await user.save();
        }
      }
    } else if (winner === 'draw') {
      ...
      for (const player of allPlayers) {
        ...
        if (user) {
          user.playerStats.totalGames += 1;
          user.playerStats.mmr += 5; // 약간의 MMR 증가
          await user.save();
        }
      }
    }
